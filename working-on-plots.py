...
def genhist(truthFileName, predictionFileName, plotname):
    ...
    plt.hist(phiTrue,bins=np.linspace(-200,50,200),histtype='step',label='phi truth') #, color="blue")
    plt.hist(phiPred,bins=np.linspace(-200,50,200),histtype='step',label='phi prediction') #, color="green")
    #plt.hist(pTTrue-pTPred,bins=np.linspace(-2,2,200),histtype='step',label='pT difference')
    plt.hist(((abs(phiTrue-phiPred))/phiTrue), bins=np.linspace(-200,50,50), histtype='step', label='resolution phi absvalue')#, color="red")
    plt.hist(((phiTrue-phiPred)/phiTrue), bins=np.linspace(-200,50,200), histtype='step', label='resolution phi') #, color="purple")
    plt.yscale('log')
    plt.xlabel('phi (deg)')
    plt.legend()
    plt.show()
    plt.savefig(f"{plotname}-pTwlayers-phi-0.01lr-01-compareav-highbins.png", format="png")
